 ...
 *
 * @see org.magiclen.magiccrypt.Base64
 * @author Magic Len
 */
public class MagicCrypt {
	...
	try {
	    ...
	    return new String(org.magiclen.magiccrypt.Base64.getEncoder().encode(encryptData), "UTF-8");
	} catch (final Exception ex) {
	    throw new RuntimeException(ex.getMessage());
	}
    }
	...
	try {
	    return decrypt(org.magiclen.magiccrypt.Base64.getDecoder().decode(str));
	} catch (final Exception ex) {
	    throw new RuntimeException(ex.getMessage());
	}
    }
